the things is here: this project works, event bus works, agent--subagent works. now what i want: 
...
final test - make wellapp here find out and run comfy server to generate image autonomosly. 
and make its own backend/frontend file, cause its nice.
and make autoscreenshots when required to look after itself.
and make video avatar to greet user, cause its cool (maybe via plugin)
tts already here, kek, but i need wired my tts server with well. 
...
and i need rag-agent, that could make rag-semantic search in picked database or datasets, but also autocreate personal tools for commercial usage, like one click research solution. thats insane and big. world will hate me fot that, cause its will destroy million of jobs. 

and video viewer(watcher) with sub-creation and analyze of video, with tools to work on video with mcp/plugins. yey. 
and inbuild git system that make it robust.
and docker plugin/mcp runner, since sandbox for autoresearch is king of solution.
and plugin that will create any superagent for mcp usage of any application by api/description/keyboard control/hardtryerror attempts just cause try ai agent can control anything and find out how to use anything and everything that was created before ai. 
and of course, all the plugins to make internet sloppy, like autoposter, bank checker, buyer, seller, email reader and etc. 

and rewired backend with typescript, kek. one day. 
